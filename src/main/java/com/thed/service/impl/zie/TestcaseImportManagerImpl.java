...
public class TestcaseImportManagerImpl extends AbstractImportManager {
    ...
    private final static Log log = LogFactory.getLog(TestcaseImportManagerImpl.class);
    ...
    protected boolean validateFileByNameChange(FileObject file, ImportJob importJob) throws IOException{
		...
		try {
			...
			//for(int sheetNo=0;sheetNo<wb.getNumberOfSheets();sheetNo++){
				...
				FormulaEvaluator evaluator = wb.getCreationHelper().createFormulaEvaluator();//As a best practice, evaluator should be one per sheet
				sheet.setDisplayGridlines(false);
	
				 int lastRow = sheet.getLastRowNum() + EXTRA_ROWS_IN_END;
				 /*if(!isSheetEmpty(lastRow)){
						continue;
				 }*/
					// Start processing the sheet
					Set<FieldMapDetail> fieldMapDetails = map.getFieldMapDetails();
	
					short nameColumn = -1, stepsColumn = -1, resultsColumn = -1,externalIdColumn=-1;
					for (Object obj : fieldMapDetails.toArray()) {
						FieldMapDetail fieldMapDetail = (FieldMapDetail) obj;
						if (ZephyrFieldEnum.NAME.equalsIgnoreCase(fieldMapDetail.getZephyrField())) {
						nameColumn = converField(fieldMapDetail.getMappedField());
						} else if (ZephyrFieldEnum.STEPS.equalsIgnoreCase(fieldMapDetail.getZephyrField())) {
						stepsColumn = converField(fieldMapDetail.getMappedField());
						} else if (ZephyrFieldEnum.RESULT.equalsIgnoreCase(fieldMapDetail.getZephyrField())) {
						resultsColumn = converField(fieldMapDetail.getMappedField());
						}else if (ZephyrFieldEnum.EXTERNAL_ID.equalsIgnoreCase(fieldMapDetail.getZephyrField())) {
				...
				String uniqueId = "";
					...
					for (int i = startPoint; i < lastRow; i++) {
						...
						if(i==startPoint){// || blockStart==true ){
							uniqueId = getCellValue(row.getCell(nameColumn),evaluator);
						}
						if(getCellValue(row.getCell(nameColumn),evaluator)!=null && uniqueId != null && !(uniqueId.equalsIgnoreCase(getCellValue(row.getCell(nameColumn),evaluator)))){
							uniqueId = getCellValue(row.getCell(nameColumn),evaluator);
		...
		/*			if (row.getLastCellNum() > maximumCount) {
						maximumCount = row.getLastCellNum();
					}*/
						...
						Cell stepsCell = row.getCell(stepsColumn);
						Cell resultsCell = row.getCell(resultsColumn);
	
						if(blockStart && !isTestcaseNameExist){
							addJobHistory(importJob, file.getName()+" Testcase name not exists at '"+(i+1)+"' row");
						}
						...
						if(isExpectedresultsExist && !isStepsExist){
							addJobHistory(importJob, file.getName()+" Invalid at "+(i+1)+" Result without step");
						}
						if(isLastRow && !isTestcaseNameExist){  //if lastRow check for testCase exists
							addJobHistory(importJob, file.getName()+" Testcase name not exists at '"+(i+1)+"' row");
						}
					}// for ends
		    ...
		    //}
			/*if (editedFile) {
				cleanFile(file, map.getStartingRowNumber(), maximumCount - 1);
			...
			String msg = ((file!=null)?(file.getName()):(""))+" Records contain invalid format/data";
			addJobHistoryAndUpdateStatus(importJob, Constants.IMPORT_JOB_NORMALIZATION_FAILED, msg);
            log.fatal("", e);
			return false;
		}
		catch (Exception e) {
			addJobHistoryAndUpdateStatus(importJob, Constants.IMPORT_JOB_NORMALIZATION_FAILED, e.getMessage());
            log.fatal("",  e);
			return false;
		}
		finally{
		...
		try {
			...
			//for(int sheetNo=0;sheetNo<wb.getNumberOfSheets();sheetNo++){
				...
				for (i = startPoint; i < lastRow; i++) {
					...
					if(i==startPoint){// || blockStart==true ){
						uniqueId = getCellValue(row.getCell(externalIdColumn),evaluator);
					}
					{
						String tempId = getCellValue(row.getCell(externalIdColumn), evaluator);
						if(!StringUtils.equalsIgnoreCase(tempId, uniqueId)){
							/*** Lets deal with old block **/
							if(!isTestcaseNameExist){
								addJobHistory(importJob, file.getName()+" Testcase name not exists in '"+uniqueId+"' block");
							}
							if(uniqueId == null || uniqueId == ""){
								addJobHistory(importJob, file.getName()+" external Identifier not exists in row(s)' "+ i +"' and/or above");	//one row before the current row. 
							}
							//reset the flag
							isTestcaseNameExist = false;
							/*** Start with the new block **/
							uniqueId = tempId;
							//In some cases, this can cause two error rows for the same block (when a empty extId block exists in the middle of the filled blocks
							if(uniqueId == null || uniqueId == ""){
								addJobHistory(importJob, file.getName()+" external Identifier not exists in '"+ (i+1) +"th' row");
							}
							blockStart = true;
						}
					}
				...
				if(!blockStart && lastRow > 2){
					if(uniqueId == null || uniqueId == ""){
						addJobHistory(importJob, file.getName()+" external Identifier not exists in '"+ i +"' row");
					}
				}
	...
	private void processColumns(Map<String, ColumnValueHolder> columns, FormulaEvaluator evaluator, 
			Row row, ArrayList<TestStepDetailBase> testsValue, int i) {
		...
		for (Map.Entry<String, ColumnValueHolder> entry: columns.entrySet()) {
            ...
            if(holder.getColumnIndex() < 0) continue;
	...
	/*
	 * Common logic in importFileForByName() and importFileById() is abstracted in importFileByChange().
	 *  
	 * (non-Javadoc)
	 * @see com.thed.service.impl.zie.AbstractImportManager#importFileForByName(java.io.File, com.thed.model.ImportJob, java.lang.Long)
	 */
	...
	@Override
	protected boolean importFileById(FileObject file, ImportJob importJob, Long userId) {
		return importFileByChange(file, importJob, userId, ZephyrFieldEnum.EXTERNAL_ID);
	}

	/*
	 * Common logic in importFileForByName() and importFileById() is abstracted in importFileByChange().
	 */
	protected boolean importFileByChange(FileObject file, ImportJob importJob, Long userId, String zephyrField) {
		...
		try {
			...
			/* Holds custom columns.
			 * Key is fieldConfig.id as String */
			...
			//for(int sheetNo=0;sheetNo<wb.getNumberOfSheets();sheetNo++){
				...
				short uniqueColumn = uniqueColumnHolder.getColumnIndex();
				...
				for (int i = startPoint; i < lastRow; i++) {
					...
					if(getCellValue(row.getCell(uniqueColumn),evaluator)!=null && uniqueId != null && !(uniqueId.equalsIgnoreCase(getCellValue(row.getCell(uniqueColumn),evaluator)))){
						uniqueId = getCellValue(row.getCell(uniqueColumn),evaluator);
						//isTestcaseNameExist = false;
						blockStart = true;
					}
				...
				addJobHistoryAndUpdateStatus(importJob, Constants.IMPORT_JOB_IMPORT_FAILED, e.getMessage());
                log.fatal("", e);
				return false;
			}
			finally{
	...
	private void truncateUpdate(ImportJob importJob, Map<String, ColumnValueHolder> columns){
		...
		for (Map.Entry<String, ColumnValueHolder> entry: columns.entrySet()) {
			...
			if (holder.getTruncateRowIndex().size() > 0) {
				truncationInfo.append("in rows " + holder.getTruncateRowIndex() + " " + holder.getFieldConfig().getDisplayName() 
				+ " was truncated to " +  holder.getFieldConfig().getLength() +" chars.\n");
			}
		}
	...
	protected boolean validateFileByEmptyRow(FileObject file, ImportJob importJob) throws IOException {
		...
		try {
			...
			FormulaEvaluator evaluator = wb.getCreationHelper().createFormulaEvaluator();//As a best practice, evaluator should be one per sheet
				...
				short nameColumn = -1, stepsColumn = -1, resultsColumn = -1;
				for (Object obj : fieldMapDetails.toArray()) {
					FieldMapDetail fieldMapDetail = (FieldMapDetail) obj;
					if (ZephyrFieldEnum.NAME.equalsIgnoreCase(fieldMapDetail.getZephyrField())) {
						nameColumn = converField(fieldMapDetail.getMappedField());
				...
				for (int i = startPoint; i < lastRow; i++) {
					...
					if (isRowNull(row)) {
						if (!blockFlag) {
							if (!(isNameExist && isStepsExist && isExpectedresultsExist)) {
								//invalidList.add(startPoint);   //here startPoint is invalid testcase row no.
								if(!isNameExist){
									addJobHistory(importJob, file.getName()+", testcase name not exists at "+(startPoint+1)+" row");
								}
							}
						}
						isNameExist = false;
	              ...
	              /*	if (row.getLastCellNum() > maximumCount) {
						...
						if (stepValue != null && !"".equalsIgnoreCase(stepValue)&& resultValue != null
								&& !"".equalsIgnoreCase(resultValue)) {
							isStepsExist = true;
							isExpectedresultsExist = true;
						}
						if (stepValue != null && !"".equalsIgnoreCase(stepValue)) {
							...
							if (resultValue != null	&& !"".equalsIgnoreCase(resultValue)) {
								isStepsExist = false;
								isExpectedresultsExist = false;
								addJobHistory(importJob, file.getName()+", Result exists without Step at "+(startPoint+1)+" row");
								isSkip = true;
							}
						}
					}
     ...
     * @param metadata
     */
	private void initializeColumns(Set<FieldMapDetail> fieldMapDetails, Map<String, ColumnValueHolder> columns, Map<String, FieldTypeMetadata> metadata) {
		/* process importable fields */
		for (Object obj : fieldMapDetails.toArray()) {
			FieldMapDetail fieldMapDetail = (FieldMapDetail) obj;
			ColumnValueHolder holder = new ColumnValueHolder();
			holder.setColumnIndex(converField(fieldMapDetail.getMappedField()));
			holder.setFieldConfig(Constants.fieldConfigs.get(fieldMapDetail.getZephyrField()));
		...
		/*String treeCrumbColumn = (String) Constants.DB_PROPERTIES.getProperty("testcase.tree.column");
		if(StringUtils.isNotBlank(treeCrumbColumn)){
			ColumnValueHolder holder = new ColumnValueHolder();
			holder.setColumnIndex(converField(treeCrumbColumn));
			//holder.setFieldConfig(fieldConfigDao.getFieldConfig(Long.parseLong(ZephyrFieldEnum.TCR_CATALOG_TREE_NAME)));
			columns.put(holder.getFieldConfig().getId().toString(), holder);
		}*/
	...
	protected boolean importFileByEmptyRow(FileObject file, ImportJob importJob,Long userId) {
		InputStream fis = null;
	...
	private void saveTestCase(ImportJob importJob, ArrayList<TestStepDetailBase> testsValue, Long userId, 
				Map<String, ColumnValueHolder> columns, int lastRowIdForThisTC ) throws Exception{
		...
		for (Map.Entry<String, ColumnValueHolder> entry: columns.entrySet()) {
			...
			// ZephyrFieldEnum.STEPS, ZephyrFieldEnum.TESTDATA, ZephyrFieldEnum.RESULT values are in testsValue
			...
			/*Commenting creatorId as its done in end */	
				...
				/* custom fields */
                populateCustomField(testcase, holder.getFieldConfig(), holder.getValue());
            }
		}
		/* Lets see if user wants to import creatorId, if not, we 'll default it to the person performing import*/
    ...
    protected void setTestCaseContents(String issueId, ArrayList<TestStepDetailBase> testsValue, Long userId)
			throws Exception {
		/*TestStep ts = new TestStep();
		ts.setLastModificationDate(new Date());
		ts.setLastModifiedBy((userId != null ? userId : 1L));
		ts.setReleaseId(releaseId);
		ts.setTcId(testcase.getId());
		ts.setSteps(TestcaseContentsUtil.convert2(testsValue));
		ts.setMaxId(ts.getSteps().size());*/
		for(TestStepDetailBase ts : testsValue){
			JiraService.saveTestStep(issueId, new TestStep(ts.getStep(), ts.getData(), ts.getResult()));
		}
	}
	...
	public boolean isFieldMapInputvalid(ImportJob importJob){
		...
		if(map.getDiscriminator().equalsIgnoreCase(Constants.BY_ID_CHANGE) ){
			if(nameColumn != -1 && stepsColumn != -1 && resultsColumn != -1 && externalIdColumn != -1){
				isValidFieldMap = true;
			}
		}
		if(map.getDiscriminator().equalsIgnoreCase(Constants.BY_EMPTY_ROW) || map.getDiscriminator().equalsIgnoreCase(Constants.BY_TESTCASE_NAME_CHANGE)){
			if(nameColumn != -1 && stepsColumn != -1 && resultsColumn != -1){
				isValidFieldMap = true;
			}
		}
		if(!isValidFieldMap){ //for jobHistory purpose
			...
			addJobHistory(importJob, "The following required fields are missing :"+ missedFiledMaps.replace(missedFiledMaps.length()-1, missedFiledMaps.length(), "."));
		}
		
		return isValidFieldMap;
	}
	
//	private void cleanFile(File file, int startRowNum, int maximumCount) throws Exception {
//		POIFSFileSystem fs = new POIFSFileSystem(new FileInputStream(file));
//		HSSFWorkbook wb = new HSSFWorkbook(fs);
//		HSSFSheet sheet = wb.getSheetAt(0);
//		evaluator = new HSSFFormulaEvaluator(sheet, wb);//As a best practice, evaluator should be one per sheet
...
//	}
//
//	private void fileupdation(File file, ArrayList<Integer> invalidList,int maximumCount) throws Exception {
//		POIFSFileSystem fs = new POIFSFileSystem(new FileInputStream(file));
//		HSSFWorkbook wb = new HSSFWorkbook(fs);
//		HSSFSheet sheet = wb.getSheetAt(0);
//		evaluator = new HSSFFormulaEvaluator(sheet, wb);//As a best practice, evaluator should be one per sheet
